...
heatmaps and adaptive group-constrained boundaries. Its 28 node features
contained only transition, reward, topology, coordinate, endpoint-distance,
slip, and discount information. The final teacher protocol removed two
...
$769.8\times$ faster than iterative teacher selection. Boundary
...
adaptive teacher passed 71/90. In a paired comparison, the GCN alone passed
eight contexts and nearest-start alone passed two. Full group audit and
fallback reduced the accepted GCN pipeline to $0.444\times$ the adaptive
teacher pipeline. A score-margin threshold calibrated for 100\% validation
...
test pass count improved from 68/90 to 81/90 (the candidate-union oracle was
85/90), at 0.00635 seconds median selection time or $656.0\times$ versus the
iterative teacher. This isolates the original count head as one source of
error, but does not establish dominance over the nonoptimal adaptive teacher.
...
further topology-holdout expansion. The complete neural experiment is therefore
...
direct risk prediction transfers the teacher's group and value guarantees. The
explicit Green operator remains our search-free proposal; the learned student
