...
group-constrained selector keeps topological, value, and stochastic distortion
budgets separate, while certified adaptive Green approximations and adaptive
feasible top-$k$ refinement reduce discovery cost without changing the claimed
feasible envelope. Lean 4 proofs cover the frozen score identity, Green-kernel
...
Operator and certified truncated implementations; (iii) group-constrained and
adaptive feasible top-$k$ discovery with an explicit score-optimality boundary;
(iv) an end-to-end value-gap decomposition that separates option restriction,
...
than one sparse full-state solve. This is a planning-compression result, not a
single-task wall-time win.
...
adaptive feasible top-$k$, reward relabeling, goal-event error, and the four-term
primitive-to-graph gap. Formalization does not turn the adaptive discovery
heuristic into a global optimizer; it fixes the meaning of the operator and its
certificates.
...
more than one full-state solve, and its benefit is clearest for repeated
planning or reward relabeling. Group constraints are only as complete as their
