...
| Does the code path generalize beyond the custom GridWorld class? | generic finite-MDP adapter, Gymnasium ToyText, discretized PointMaze, optional MiniGrid symbolic BFS | `experiments/finite_mdp_adapter.py`, `experiments/run_general_env_benchmark.py`, `experiments/output/general_env_benchmark/` | smoke implemented: 56 rows |
...
5. General finite-MDP smoke tests now separate interface portability from the main finite-grid compression claim. Taxi is currently an informative failure mode because spatial boundary compression alone does not preserve passenger/destination task variables.
