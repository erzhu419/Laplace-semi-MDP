...
4. A group-constrained robust selector that keeps topology, value-gradient, and stochastic-transition distortions separate instead of collapsing them into one scalar risk.
5. A graph-SMDP evaluation suite comparing full VI/PI, exact/surrogate/group RD graphs, eigenoption-style spectral baselines, betweenness bottlenecks, random/coverage landmarks, and tabular option baselines.
6. A Lean 4 proof layer covering the frozen finite-difference identity, Green-kernel legality, truncated Neumann tails, bits-distortion Taylor bounds, Bellman contraction, residual-to-value-gap bounds, and top-set certificates.
